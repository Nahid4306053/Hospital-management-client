...
export default function Banner() {
  ...
  return (
    ...
    >
      <div className="">
        <Swiper spaceBetween={30} centeredSlides={true}   autoplay={{ delay: 2500, disableOnInteraction: false, }} pagination={{ clickable: true, }} direction="vertical"  modules={[Autoplay, Pagination]} className="mySwiper  max-h-[600px] md:max-h-[700px]  overflow-x-hidden mt-0  md:mt-28 2xl:mt-0 container mx-auto   w-full h-full" >
                    ...
                    <div className="slideItems pt-5 flex   justify-between items-center">
                      <div className="descriptions p-10 flex justify-center flex-1">
                        <div className=" space-y-6 ">
                          <h3 className="lg:text-2xl text-sl text-white"> {ele.slogan} </h3>
                          <h1 className="text-3xl lg:text-5xl text-white font-bold"> {ele.service_title} </h1>
                          <p className="text-xs md:text-lg  text-white max-w-xl"> {ele.description}</p>
                          <div>
                            <button className="btn btn-outline text-white hover:border-white px-14 lg:mt-7 border-2 rounded-none hover:bg-transparent md:btn-md  btn-sm"> Details </button>
                          </div>
                        </div>
                      </div>
                      <div className="banner md:block hidden h-full flex-1">
                        <img className="h-[700px]  mx-auto" src={ele.banner} alt="" />
                      </div>
                    </div>
                  </SwiperSlide>
           )
